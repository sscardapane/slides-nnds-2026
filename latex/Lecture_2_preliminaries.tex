...
\begin{frame}[fragile]{Descent direction}
	...
	Without loss of generality, we restrict to unit directions ($\lVert \vc{p}_t \rVert = 1$). The rate of change is given by the directional derivative (recalling $\langle \vc{a}, \vc{b} \rangle = \lVert \vc{a} \rVert \lVert \vc{b} \rVert \cos(\theta)$ for vectors $\vc{a}, \vc{b}$ and angle $\theta$ between them):
	%
	\begin{equation*}
	...
	\end{equation*}
	%
	The above quantity is minimized when $\cos(\theta) = -1$, which happens if $\theta=\pi$:
	%
	\begin{equation*}
		\vc{p}_t \propto
	...
	\end{equation*}
	%
	This is the \myalert{steepest descent direction}. In general, anything with $\cos(\theta) < 0$ is a descent direction.
...
\begin{frame}{Adam}
	...
	\begin{equation*}
		\begin{aligned}
			...
			\widehat{\vc{v}}_t
			=
			\frac{\vc{v}_t}{1-\beta_2^t}\,, &   & \text{\footnotesize correction,}    \\
			{\color{darkred}
			\vc{\theta}_t
			                                & =
			\vc{\theta}_{t-1}
			...
			{\sqrt{\widehat{\vc{v}}_t}+\varepsilon}}
			                                &   & \text{\footnotesize update}.
		\end{aligned}
	\end{equation*}

	\vspace{-0.8em}
	\footnotesize
	$\vc{m}_t$ smooths the direction as in momentum; $\vc{v}_t$ rescales the coordinates; the hats correct the bias from $\vc{m}_0=\vc{v}_0=\vc{0}$. $\beta_1,\beta_2\in[0,1)$ and all operations involving $\vc{v}_t$ are element-wise.
